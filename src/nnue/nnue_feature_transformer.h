...
using psqt_vec_t = int32x4_t;
    ...
    #define vec_mulhi_16(a, b) vmulq_s16(a, b)  // incomplete
    #define vec_zero() \
        vec_t { 0 }
    #define vec_set_16(a) vdupq_n_s16(a)
    #define vec_max_16(a, b) vmaxq_s16(a, b)
    #define vec_min_16(a, b) vminq_s16(a, b)
    #define vec_slli_16(a, b) vshlq(a, vec_set_16(b))
inline vec_t vec_packus_16(vec_t a, vec_t b) {
    const int8x8_t  shifta    = vqshrn_n_s16(a, 0);
    const int8x8_t  shiftb    = vqshrn_n_s16(b, 0);
   ...
   public:
    ...
    std::int32_t transform(const Position&                           pos,
        ...
        for (IndexType p = 0; p < 2; ++p)
        {
            ...
            {
                const vec_t sum0a =
                  vec_slli_16(vec_max_16(vec_min_16(in0[j * 2 + 0], One), Zero), 8);
                const vec_t sum0b =
                  vec_slli_16(vec_max_16(vec_min_16(in0[j * 2 + 1], One), Zero), 8);
                const vec_t sum1a = vec_slli_16(vec_min_16(in1[j * 2 + 0], One), 1);
                const vec_t sum1b = vec_slli_16(vec_min_16(in1[j * 2 + 1], One), 1);
